...
def main(): 
    ...
    max_base = top[0][1]
    with open('new_combination', 'w') as f:
        for i, j in top:
            print >>f, i.encode('U8'), j/max_base

    print 'writing....'
    top = [ [i, j] for i,j in word_freedom.iteritems()]
    top.sort(key=lambda x:x[1], reverse=True)
    max_base = top[0][1]
    with open('new_freedom', 'w') as f:
        for i, j in top:
            print >>f, i.encode('U8'), j/max_base

    print 'writing....'
    top = [ [i, j] for i,j in word_sep_freq.iteritems()]
    top.sort(key=lambda x:x[1], reverse=True)
    max_base = float(top[0][1])
    with open('new_sep_freq', 'w') as f:
        for i, j in top:
            if len(i) > 1:
                print >>f, i.encode('U8'), j/max_base
